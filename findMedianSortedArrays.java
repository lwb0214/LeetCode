...
class Solution {
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        // 两个数组长度之和为奇数时，按照分割线的左边比右边多一个元素的要求，
		// 此时在长度之和上+1，就会被2整除，会在原来的数的基础上+1，
		// 于是多出来的那个1就是左边比右边多出来的一个元素
        int totalLeft = (leftLength + rightLength + 1) >>> 1;
        ...
        while (left < right) {
            ...
            // left和right最小差距是1，此时下面的计算结果如果不加1会出现i一直=left                  
			// 的情况，而+1之后i才会=right
            // 于是在left=i的时候可以破坏循环条件，其次下标+1还会保证下标不会越界，                 
			// 因为+1之后向上取整，保证了
            // i不会取到0值，即i-1不会小于0
            // 此时i也代表着在一个数组中左边的元素的个数
            int i = (left + right + 1) >>> 1;
            // 第一个数组中左边的元素个数确定后，用左边元素的总和-第一个数组中元素的总和=
			// 第二个元素中左边的元素的总和
            // 此时j就是第二个数组中左边的元素的个数
            int j = totalLeft - i;
            // 此处用了nums1[i - 1] <= nums2[j]的取反，当第一个数组中分割线的左边的值大于
			// 第二个数组中分割线的右边的值
            // 说明又指针应该左移，即-1
